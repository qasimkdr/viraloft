...
// The backend already applies Viraloft's 20% commission exactly once.
const displayRateUSD = (service) => Number(service?.markupRate || 0);
...
export default function Services() {
  ...
  useEffect(() => {
    ...
    httpPublic.get('/services/public/categories', { signal: controller.signal })
      .then((response) => {
        if (!mounted) return;
        const list = Array.isArray(response.data) ? response.data : ['All'];
        setCategories(list.length ? list : ['All']);
      })
      .catch(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      try {
        ...
        if (requestError?.name !== 'CanceledError' && requestError?.name !== 'AbortError') {
          setServices([]);
          setEndReached(true);
          setError(requestError?.response?.data?.message || 'Unable to load services right now. Please try again.');
        }
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(async ([entry]) => {
      ...
      try {
        ...
        // Keep already loaded services visible if a load-more request fails.
      } finally {
  ...
  const categoryOptions = useMemo(() => {
    ...
    return Array.from(new Set(['All', ...categories.filter((item) => item !== 'All'), ...visible]));
  }, [categories, services]);
          ...
          <div className="mx-auto max-w-7xl">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 max-w-2xl text-base leading-8 text-slate-400 sm:text-lg">
                Search the live catalog, compare service limits and review customer pricing with Viraloft&apos;s service commission already included.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="px-4 py-8 sm:px-6 sm:py-10">
          <div className="mx-auto max-w-7xl">
            ...
            {loading ? (
              ...
              <div className="grid min-h-52 place-items-center rounded-2xl border border-red-400/20 bg-red-400/[.05] px-6 text-center text-red-200">{error}</div>
            ) : services.length === 0 ? (
                      ...
                      <li key={service.service ?? `${service.name}-${index}`} className="interactive-3d flex min-h-72 flex-col rounded-2xl border border-white/10 bg-gradient-to-br from-[#101a30] to-[#0a1020] p-5 shadow-xl shadow-black/10">
                        ...
                        <div className="mt-auto pt-5">
                          <div className="mb-3 flex items-end justify-between gap-2">
                            <span className="text-xs text-slate-500">Customer rate · 20% included</span>
                            <strong className="text-base text-white">{currency} {money(rate, currency)} <small className="font-medium text-slate-500">{perItem ? '/unit' : '/1k'}</small></strong>
                          </div>
