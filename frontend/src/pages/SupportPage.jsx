...
export default function SupportPage() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    await api.post("/api/tickets", { subject, message, priority: priorityNew });
    setSubject("");
    setMessage("");
  ...
  const sendMessage = async (ticketId) => {
    ...
    await api.post(`/api/tickets/${ticketId}/message`, { text: newMessage });
    setNewMessage("");
  ...
  const searchUsers = async () => {
    ...
    const r = await api.get("/api/users/search", { params: { q: qq } });
  ...
  const sendDM = async () => {
    ...
    try {
      setSendingDM(true);
      await api.post("/api/staff/messages", { userId: dmTo._id, text: dmText });
